...
# `reasoning effort: high` is what a real banner prints — a bare "high" that
# must not read as a severity marker (codex, #61 pass 3).
...
echo "── a short real review with 'Reviewed 3 files' that quotes a wall phrase is NOT a wall (codex L, #61 p3) ──"
codex_shim 0 "Reviewed 3 files. The grep for 'purchase more credits' is fine. No issues."
run "$T/o6"
assert_eq "short review with a reviewed-N marker stays rc 0" "$(jq -r .exit_code "$T/o6/codex.meta.json")" "0"
[[ ! -f "$T/o6/codex.quota_exhausted" ]] && ok "…and no sentinel" || bad "sentinel written for a short real review"
