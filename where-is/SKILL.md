...
1. **symbol** — single token matching `^[A-Za-z_$][A-Za-z0-9_$]*(\/[A-Za-z_$][A-Za-z0-9_$]*)*$`, *or* PascalCase/camelCase with no spaces. Slash-separated `Class/method` paths are allowed (Serena's `name_path` format) — checked before rule 2, so this shape is a symbol even though it also contains `/`. → emit canonical name and ask the model to call Serena MCP tools.
2. **path** — query contains `/`, ends in a known code extension (`.ts .tsx .js .jsx .mjs .cjs .json .md .yml .yaml`), or contains glob chars (`* ? [`), and didn't already match rule 1. → `scripts/fs_walk.sh`
3. **pattern** — query contains parens, angle-brackets, or one of the ast-grep marker tokens (`=>`, `as any`, `useEffect(`, `useState<`, `<$`, `$$$`). → `scripts/where-is.sh --kind pattern`
