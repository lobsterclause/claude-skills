...
- kimi (direct Moonshot, no CLI since 2026-09-03): `export MOONSHOT_API_KEY=...` or `mkdir -p ~/.config/moonshot && (umask 077; printf '%s\n' 'sk-...' > ~/.config/moonshot/key)`. The Kimi Code CLI is no longer used — its plan mode re-sent every prompt 3–25 times and drained ~$20/day with nothing stamped in the runlog
...
- kimi27 + kimi3 (Kimi k2.7-code and Kimi K3, direct Moonshot): the same Moonshot platform key as the kimi baseline — one key lights all three, one balance bills all three
...
For `kimi` the usual cause is a missing Moonshot key (`MOONSHOT_API_KEY` or
`~/.config/moonshot/key`) or an exhausted balance. For `codex` the usual cause
is PATH, not a missing install. `codex` is an npm global
...
### 2.5. What the text-only seats can see (default: the whole changed files)

Most of the fleet cannot read files. `codex` roams the worktree as an agent and the two agy laps get the repository mounted with file-reading tools, but **everything else** — `kimi`, the OpenRouter pool, `kimi27`/`kimi3` — is a single chat completion with the diff pasted in and no tools. A seat that can only see a hunk is asked a question its input cannot answer, and every recurring false-P0 shape on record is a claim about code *outside* the hunk (a portability claim never run, "this invalidates the suite", a case the enclosing function already handles). Adjudicated precision on 2026-08-24 lined up with the tiers exactly: `codex` 25/26 with file access, `qwen` 0/4 and `deepseek` 0/3 without (issue #69).

So by default `run_reviewers.sh` appends the **whole post-change contents of every file the diff touches** after the `<diff>` in every text-only seat's prompt (`--context-mode files`, the default; deleted files and binaries omitted; ordered by churn; capped at `CROSS_REVIEW_CONTEXT_BUDGET_BYTES`, default 300000, with any file that does not fit omitted whole and **named in the prompt** so the seat knows where its view ends). The diff still says what changed; the files say what surrounds it. `--context-mode diff` (or `CROSS_REVIEW_CONTEXT_MODE=diff`) restores the hunk-only prompt if you need to compare, or when the budget is the bottleneck. Every seat's `meta.json` records what it could see as `context_access` (`agent` | `workspace_read` | `file_context` | `snapshot` | `diff_only`) — step 4's scorer reads that to weight convergence (see `--meta-dir` there).

The tradeoff is explicit: whole-file context costs more tokens per text-only seat, every round. It buys back the scarce resource — the adjudication attention a human or agent spends building context to discard a false P0.

### 2.5b. Tools for the chat-completions seats — the wrapper-owned tool loop (default: learned per seat)

"Text-only" was a choice, not a constraint. Checked 2026-08-27: every pinned OpenRouter model advertises `tools` in `supported_parameters`, Moonshot's endpoint is OpenAI-compatible, and the reasons the wrapper gave for running them single-shot were stale. Rather than teach each vendor CLI a permission dialect (the agy soft-deny saga; kimi's adapter quirks), **the wrapper is the harness** (`scripts/lib_tool_loop.sh`): `run_reviewers.sh` drives a bounded multi-turn conversation with a fixed toolset that it executes itself, in the parent's sandbox, identically for the whole OpenRouter pool, `kimi27`/`kimi3`, and every first-party lane that lands on OpenRouter as a `fallback` — which is how codex keeps its agent-ness when its own lane is down.

Three arms, `--tool-mode` / `CROSS_REVIEW_TOOL_MODE`:

| arm | what the seat gets | `context_access` |
|---|---|---|
| `off` | the pre-2026-08-27 single shot (diff + whole changed files) | `file_context` |
| `read` | `read_file` (a range of any tracked file, post-change), `search` (`git grep -n -E`), `list_files` | `tool_read` |
| `check` | `read` + `run_check` — runs the repository's **own** declared verify entrypoint (`CROSS_REVIEW_CHECK_CMD` → `.claude/verify.sh` → `package.json` `verify` → `Makefile verify`/`check`) once per round, cached across seats | `tool_check` |

**The model never chooses a command.** `run_check` is the repo's command or nothing; a repo with no entrypoint has no `check` arm (a request degrades to `read`, basis `…:no_check_entrypoint`) — and neither does a repo the operator has not vouched for: `run_check` executes a command the **reviewed branch** controls, in the wrapper's own environment (HOME, keys, network), so the learner may only pick `check` when `CROSS_REVIEW_TOOL_CHECK_TRUST=1` is set; an explicit `CROSS_REVIEW_TOOL_MODE=check` vouches for that round. Leave both unset for pull requests from strangers. The check runs under the smaller of `CROSS_REVIEW_CHECK_TIMEOUT_S` and what is left of the lane's budget. `read_file` serves tracked, non-symlink, text files inside the repo root only, capped per call (400 lines) and per seat (`CROSS_REVIEW_TOOL_READ_BUDGET_BYTES`, default 400000), refuses the `worktree.sh` secret-path pattern outright and redacts its secret-content pattern from what it returns. `search` takes the pattern as an argument (never eval'd), refuses a `path_glob` that matches the secret-path pattern, and drops every hit whose path matches it. The step budget (`CROSS_REVIEW_TOOL_MAX_STEPS`, default 8) ends with one forced final turn (`tool_choice: none`); so does the wall budget, as soon as two more turns as slow as the slowest so far would not fit (`tool_stats.wall_forced`). A tool-armed lane's wall budget is the seat's `timeout_s` plus `(max_steps + 1) × prompt_KB / wall_kb_per_s` seconds (every turn re-sends the whole prefix), capped at `wall_cap_s` and logged as `budget scaled`; an explicit `--timeout` / `--timeout-<seat>` is honoured verbatim (#159). Whole-file paste (step 2.5) stays on underneath by default — tools add on top; `CROSS_REVIEW_TOOL_CONTEXT=diff` drops it when tools are on. Every turn's request/response is kept (`<seat>.turn.<n>.*.json`) and every call is one line in `<seat>.tools.jsonl`.

**`auto` (the default) is self-learning, per seat.** `scripts/tool_policy.sh --reviewer <seat>` picks the arm from the ledgers this skill already keeps — `runlog.jsonl` (each run's `tool_stats.mode`, status, cost, findings counts) and `finding_events.jsonl` (unanchored findings) — the same way `leaderboard.sh` scores. Per (seat, arm): `reward = 0.6·r_q + 0.4·r_ok − cost_lambda·cost_usd`, where `r_q` is the run's precision proxy `(findings − dropped − 0.5·unanchored)/findings` (0.5 when the run found nothing — uninformative), then UCB `mean + ucb_c·√(ln(N+1)/(n+1))` with an optimistic prior for untried arms so each gets sampled, and a demotion rule (≥ `min_samples` runs and reliability < 0.5 → the loop breaks that model, arm out). Runs the provider never judged are not samples: `failure_kind` `provider_billing` / `provider_rate_limited` / `provider_auth` / `transport_error` / `quota_exhausted` (classified by `run_reviewers.sh` and the loop from the API error body — a 402 during an OpenRouter balance outage must not demote whichever arm was drawn) are reported as `excluded_runs` and skipped; a generic `provider_error` ("tools not supported on this route") *is* a sample, since that is what demotion is for. Ties go `read > check > off`. Deterministic, no hidden state: the decision and its basis (`override` | `pinned` | `learned` | `default`) are stamped into the seat's `meta.json` → runlog, so the next round sees it and `tool_policy.sh --reviewer <seat> --explain` (or `--all --mode table`) reproduces any decision. Historical single-shot rows are the `off` arm's samples, so the learner starts with a real baseline, not a blank.

**Tunables** live in `reviewer_profiles.json` `_synthesis_rules.tool_policy` (defaults for the numbers above, `default_mode`, `ucb_c`, `min_samples`, `cost_lambda`, `window`) and are overridden per run by the `CROSS_REVIEW_TOOL_*` / `CROSS_REVIEW_CHECK_*` env; a seat's own `tools: {mode, max_steps, read_budget_bytes}` pins it and stops learning for it. `--tool-mode off` reproduces the pre-feature round byte-for-byte.

`score_findings.sh` weights `tool_read`/`tool_check` at 1.0 — a seat that read the surrounding code or ran the suite is a full capability vote — and the scorer's independence rules (step 4) apply unchanged: two seats sharing a *model* still share a blind spot, tools or not.

**Optional: a `repomix-handoff` snapshot instead.** For a diff whose changed files blow the budget, or when a seat needs a curated slice (imports one hop out, a path-scoped view), build a token-budgeted snapshot and pass `--snapshot-dir "$run_dir"` to `run_reviewers.sh` (step 3): for each dispatched reviewer `<r>` it looks for `$run_dir/snapshot-<r>.md`, then `.xml`, then `.txt` (first match wins) and, if found, uses that file's contents as the code-context block **instead of** the diff and the whole-file block — a snapshot is the seat's entire view, so it is not doubled up. A reviewer with no matching file keeps the default path. This is a sibling skill, not a hard dependency; if `repomix-handoff` is missing, skip it.
...
if [ -x ~/.claude/skills/repomix-handoff/scripts/detect_repomix.sh ] && \
  ...
  # Loop over THIS ROUND'S roster, not the full fleet. handoff.sh's --reviewer
  # presets are codex|gemini|kimi|claude only — map every other seat onto the
  # preset with the matching context budget (an unknown --reviewer is a usage
  # error, exit 2, and the loop would produce nothing).
  for r in $(echo "$roster" | tr ',' ' '); do
    ...
    bash ~/.claude/skills/repomix-handoff/scripts/handoff.sh \
      --reviewer "$preset" --base <base-branch> \
      --output "$run_dir/snapshot-$r.md" >/dev/null
  done
fi
```

**agy-lap size cap**: agy's `-p` is argv-only (no stdin/file prompt mode), so for antigravity/gemini-pro a snapshot whose assembled prompt would exceed the 100KB argv guard is refused with a stderr WARN and that lap falls back to the raw diff — in practice keep agy snapshots under ~90KB, or accept the fallback. **codex is the one exception**: `codex exec review --base` does its own diffing internally and has no text-embedded diff to swap out, so a `snapshot-codex.*` file is ignored — if you need to bound codex's input too, drop `--base` and build a custom prompt by hand (see `references/cli_flags.md`).

| Reviewer | handoff preset | Style | Max tokens |
|---|---|---|---|
| codex       | codex  | XML      | 160k |
| antigravity | gemini | Markdown | 1M   |
| gemini-pro  | gemini | Markdown | 1M   |
| kimi        | kimi   | Markdown | 200k |
| OpenRouter pool / kimi27 / kimi3 | kimi | Markdown | 200k |

**Skip the snapshot** if `warn_secrets: true` is still unresolved — bound or unbound, packed snapshots still contain whatever you pack. (The default whole-file block has the same property: it embeds the changed files as they are.)
...
- `agy --model "Gemini 3.7 Flash (High)" --sandbox -p '<prompt>'` — the **antigravity** reviewer (fast lap)
- `agy --model "Gemini 3.1 Pro (High)" --sandbox -p '<prompt>'` — the **gemini-pro** reviewer (deep lap; same `agy` binary, different model)
- `curl` against the **direct Moonshot** chat-completions endpoint (`api.moonshot.ai/v1`, model `kimi-k2.7-code` from the profile) — the **kimi** baseline. Same body and tool loop as the OpenRouter pool below; cli label `moonshot` in meta.json. (Until 2026-09-03 this was the Kimi Code CLI in `--plan --print` mode, which turned out to be a 3–25-step agent loop re-sending the whole prompt each step at ~$20/day with no cost stamped.)
- `curl` against the OpenRouter chat-completions API with the same diff + whole-changed-files prompt as kimi, plus the wrapper-owned tool loop when the seat's learned arm is `read`/`check` (step 2.5b) — the whole rotation pool: **glm** (`z-ai/glm-5.3`), **deepseek** (`deepseek/deepseek-v4-pro-0813`), **mimo** (`xiaomi/mimo-v2.5`), **minimax** (`minimax/minimax-m3`), **qwen** (`qwen/qwen3-coder-next`), **devstral** (`mistralai/mistral-large-2512`), **laguna** (`poolside/laguna-s-2.1`), **kat** (`kwaipilot/kat-coder-pro-v2.5`), **north** (`cohere/north-mini-code:free`), **nemotron** (`nvidia/nemotron-3.5-lightning:free`), **spark** (`meta/muse-spark-1.2`), **seed** (`bytedance-seed/seed-2.0-code`), **grok** (`x-ai/grok-4.6`), **longcat** (`meituan/longcat-2.0`), **inkling** (`thinkingmachines/inkling`)
...
Outputs land at `<reviewer>.{stdout,stderr,meta.json}` (OpenRouter reviewers also write `request.json`/`response.json` for audit); each `meta.json` carries `model`, `cli`, and `context_access` (what the seat could see: `agent` for codex, `workspace_read` for the agy laps, `file_context` / `snapshot` / `diff_only` for the text-only lanes); the chat lanes also stamp `cost_usd`, `tokens_prompt` / `tokens_completion`, the prompt-cache counters `tokens_cached` / `tokens_cache_write`, and `upstream_provider` (the host OpenRouter routed to — see "Prompt caching" below) — and, for the agy laps, a `failure_kind` field (`quota_exhausted` | `agy_panic` | `empty_output` | null). A first-party lap that fails on an ACCOUNT wall is re-run over OpenRouter when its profile enables `or_fallback` (meta then carries a `fallback` object with `used`/`succeeded`); anything else drops out and its meta says why. The wrapper returns when all are done.
...
3. Apply the change to the working tree (edit the file directly). Do **not** stage or commit yet — the gate below inspects the real, unstaged diff, not a hypothetical one.
4. **Suspicious-fix gate.** Immediately after applying, capture the actual diff of just this edit and run it through `verify_fix_safety.sh` before it goes anywhere near a commit:
   ```bash
   fix_diff_file="$run_dir/fix-${finding_id}.diff"
...
- **antigravity** (Gemini 3.7 Flash, fast lap): Uses `agy --model "Gemini 3.7 Flash (High)" --sandbox -p '<prompt>'`. `--sandbox` plus a "do not edit" prompt instruction keeps it read-only. Needs an explicit review prompt (see `references/review_prompt.txt`). Replaces the retired standalone `gemini` CLI. **Gotcha:** `agy --model` accepts only the exact `agy models` display string and **silently falls back to Flash** on a typo — never errors. Auth: `agy login` once (Google OAuth). **Empty output has two causes, not one**: check `failure_kind` in the meta.json — `quota_exhausted` (shared Google "Individual quota", 429, ~2-day reset; agy exits 0 with empty stdout in ~5s and only the `.agy.log` says why) means the lap is benched until the reset, while `empty_output` usually means expired auth (`agy login`). Don't re-auth for a quota problem. Since 2026-08-22 a quota/auth wall re-runs this lap over OpenRouter against the same Gemini model (recorded as status `fallback`, never `ok`); without `or_fallback` it drops out of the round.
- **gemini-pro** (Gemini 3.1 Pro, deep lap): Same `agy` binary, just `--model "Gemini 3.1 Pro (High)"` and a longer default timeout (900s). Pro reasons deeper and slower; for tiny diffs the Flash lap alone is often enough. Migrated off the standalone `gemini` CLI in the 2026-06-18 sunset. **Known upstream bug (agy ≤1.0.15):** a SIGSEGV panic in agy's `RunCommandHandler` — exit 2, ~20–45s, empty output, `panic: runtime error` in the `.agy.log`. Hits the Pro lap far more than Flash (Pro roams files/commands harder). Flaky, not deterministic: the wrapper retries agy once; since 2026-08-22 an `agy_panic` (like a quota/auth wall) then re-runs this lap over OpenRouter against the same Pro model, recorded as status `fallback`, never `ok`. Without `or_fallback` configured it drops out of the round.
- **OpenRouter pool** (glm / deepseek / mimo / minimax / qwen / devstral / laguna / kat / north / nemotron / spark / seed / grok / longcat / inkling): No CLI — the wrapper `curl`s the OpenRouter chat-completions API with the diff inlined (8000-line cap, `<diff>` fencing and injection defusal identical to kimi) followed by the whole changed files in a `<files>` block (`--context-mode files`, the default since 2026-08-26 — see step 2.5). Since 2026-08-27 these seats also get the wrapper-owned tool loop (step 2.5b): `read_file`/`search`/`list_files`, and `run_check` where the repo declares a verify entrypoint, executed by the wrapper; the arm is learned per seat by `tool_policy.sh` and stamped into `meta.json` as `tool_policy`/`tool_stats`. A seat whose tool turns misbehave (malformed calls, loops) demotes itself back to single-shot after `min_samples` failed runs — check `tool_policy.sh --reviewer <seat> --explain` before blaming the provider. **Prompt caching** (2026-08-28): every host that caches does so on the exact request prefix, so the prompt is ordered stable-first — instructions, then the diff and whole files, then the per-round `diff --stat` summary and notes, then the JSON contract last — and the tool loop pins turns 2+ to the upstream host that answered turn 1 (`provider.order`, fallbacks allowed; `CROSS_REVIEW_TL_PIN_PROVIDER=0` disables) because a mid-loop re-route throws the cached prefix away (kimi k2.7 lost a 52k-token cache on turn 4 when OpenRouter moved it Ambient → Inceptron, PR #117). Measured before the change: turns ≥2 hit 93% on GPT-5.6 / 67% Grok / 53% Kimi; first turns ≈0% on every model because only the ~700-token header preceded the summary. `analyze_runlog.sh --mode report` now prints `cache_hit=<pct> (<n> runs)` per seat from `tokens_cached / tokens_prompt` — that column, not a hunch, is how to tell whether caching is working. Hosts that never cache (kat on StreamLake, minimax on Parasail, kimi-k3 on DigitalOcean at time of writing) will sit at 0% regardless; the Moonshot-direct lane reports no cache fields at all. All fifteen require `$OPENROUTER_API_KEY` or `~/.config/openrouter/key`. Each is its own provider vote: Zhipu, DeepSeek, Xiaomi, MiniMax, Alibaba, Mistral, Poolside, Kuaishou, Cohere, NVIDIA, Meta, ByteDance, xAI, Meituan, Thinking Machines Lab. Provenance caveats: glm/deepseek/mimo/minimax/qwen/kat are China-origin providers, devstral is France (Mistral), laguna is US (Poolside), north is Canada (Cohere), nemotron is US (NVIDIA), spark is US (Meta), seed is China (ByteDance), grok is US (xAI), longcat is China (Meituan), inkling is US (Thinking Machines Lab) — all routed through OpenRouter (US); surface for security-sensitive repos, same as the kimi caveat. `north` and `nemotron` are `:free` OpenRouter routes — zero marginal cost, but free routes can be rate-limited and may have different data-retention terms than paid. (`fugu` / Sakana Fugu Ultra held a trial seat here 2026-07-01→02; cut after 2 samples on cost: $5/M in, $30/M out — ~100× the pool — with hidden orchestration tokens billed as output. See PR notes. `qwen`/`devstral`/`laguna`/`kat` joined 2026-07-02 as rookies to restore lab diversity. `spark` — Meta Muse Spark 1.1 — joined 2026-07-19, per Gabriel. 2026-08-14 refresh: `seed` (ByteDance Seed 2.0 Code) and `grok` (xAI Grok 4.6) joined as `draw_boost` 2.5 rookies — both labs were unrepresented; `deepseek` moved V4 Flash → V4 Pro after the Flash seat scored 52 with 2 disproven findings, `kat` V2 → V2.5, `nemotron` Ultra 550B → 3.5 Lightning. Pre-2026-08-14 leaderboard rows for deepseek and nemotron describe different models. 2026-08-22 refresh: `longcat` (Meituan LongCat 2.0) and `inkling` (Thinking Machines Inkling) joined as `draw_boost` 2.5 rookies — both labs were unrepresented; `glm` moved 5.2 → 5.3, `spark` 1.1 → 1.2, and the `antigravity` lap Gemini 3.5 Flash → 3.7 Flash. Those three are point releases within the same model family, so they did NOT get a rookie `draw_boost` — unlike the 2026-08-14 deepseek/nemotron swaps, which changed tier and model outright. Rejected in the same sweep and worth not re-litigating: `stealth/ox-alpha` (free, coding-pitched, but a cloaked model cannot be certified independent — if it is a model already seated, two seats share one brain and their agreement reads as convergence) and `sakana/sakana-namazu` (built on Kimi K2.6, so it collides with the three Moonshot seats).)
...
- **kimi3** (Kimi K3, direct Moonshot): Same lane as kimi27 — `api.moonshot.ai/v1`, model id `kimi-k3`, same Moonshot platform key and shared balance. Moonshot's flagship (2.8T MoE, 16/896 experts active, 1M context), released 2026-07-16; added as a rotation seat 2026-07-18 per Gabriel, two days later. Full open weights weren't out yet at add-time (expected 2026-07-27) — this seat hits the hosted API, not local weights. Carried `draw_boost: 2.5` while earning leaderboard data; **retired to 1.0 on 2026-08-22 per Gabriel** — enough samples gathered, same precedent as kimi27's retirement on 2026-07-12. **Same provider as kimi/kimi27**: kimi+kimi27+kimi3 agreement counts as one provider vote at synthesis. China-origin provider (Moonshot) — same caveat as kimi.
- **kimi** (Kimi k2.7-code, direct Moonshot): No CLI since 2026-09-03 — the same curl chat-completions body as the OpenRouter pool against `api.moonshot.ai/v1`, model `kimi-k2.7-code` pinned in `reviewer_profiles.json` (`.kimi.model`), Moonshot platform key from `MOONSHOT_API_KEY` or `~/.config/moonshot/key`. One request per review; `tokens_prompt`/`tokens_completion` are stamped from the response and `cost_usd_estimated` from the profile's `pricing` ($0.95/M in, $4.00/M out at the cache-miss rate), so the seat is finally visible in `runlog.jsonl`. File access comes from the wrapper-owned, budgeted tool loop (step 2.5b) when the seat's learned arm is `read`/`check`. Why the CLI went: `kimi --plan --print` was believed to be single-turn and tool-less; its session logs showed every review running ≥3 API steps (plan file, ExitPlanMode, answer) and up to 25 after compaction, re-sending a 40K–200K-token prompt each time — 15.8M tokens and ~$20 in one day (2026-09-03), none of it stamped. Note: kimi sends code to a China-origin provider — surface that to the user for security-sensitive repos.
...
- **Gemini laps both empty in ~5s**: the shared Google "Individual quota" is exhausted (429). The wrapper stamps `failure_kind: "quota_exhausted"` + the reset ETA in meta.json and writes an `agy.quota_exhausted` sentinel that spares retries and the factcheck pass (the concurrent sibling lap usually burns its own ~5s call first — 2s stagger vs ~5s detection). Both laps re-run over OpenRouter against the same Gemini models when `or_fallback` is enabled (status `fallback`, never `ok`), and otherwise drop out until the quota resets (~2-day cadence); report that honestly rather than re-running, and let rotation fill the roster. Do NOT `agy login` for this — it's not an auth problem. (The selector also down-weights a lap whose latest run was a quota failure.)
- **gemini-pro exits 2 in ~30s with empty output**: agy's SIGSEGV panic (upstream bug, `panic: runtime error` in the `.agy.log`). The wrapper retries agy once, then re-runs the lap over OpenRouter against the same Gemini model when the seat has `or_fallback` enabled (status `fallback`, never `ok`, with the primary-needs-attention warning); without it the lap drops out. The panic is matched structurally, not by text -- a client-side crash is one of the few failures a different rail genuinely fixes. If it recurs across runs, check for a newer agy release.
...
**Mutation-recall ground truth (#116):** a planted (synthetic) defect is a known-answer drill — `plant_mutation.sh` mutates one added line of a real diff via a deterministic operator table (`references/mutation_operators.json`), and `grade_planted.sh --emit-events` scores which roster seats caught it by location, appending `planted`/`caught`/`missed` events to `finding_events.jsonl`. These rounds must never leak into production scoring: `append_runlog.sh --synthetic` stamps the runlog row `synthetic: true`, and — independently, so a forgotten flag can't slip through — both `append_runlog.sh` and `leaderboard.sh` auto-detect a `planted` event on the run_id and treat the row as synthetic either way (WARNing on stderr). `leaderboard.sh --mode report` prints a **mutation recall** section computed only from these rounds — per seat, `caught / (caught + missed)`, broken down by the planted operator's `class` plus an `all` bucket, with mean `severity_accuracy` over caught entries; `--include-recall <weight>` (0 < weight ≤ 1) optionally folds a seat's overall recall into its score everywhere, including the draw. A weekly CI job (`.github/workflows/cross-review-planted-weekly.yml`, `cross-review/ci/planted_round.sh`) runs one drill against the OpenRouter-only roster and uploads the ledger deltas as a build artifact — it does not commit them back; fold a drill's results into the live ledgers by hand if you want them counted.
