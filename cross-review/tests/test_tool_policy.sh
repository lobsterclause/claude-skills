...
echo "── check availability: entrypoint AND operator trust ──"
D="$(decide glm "$T/withcheck")"
assert_eq "entrypoint alone does NOT arm check (untrusted branch)" "$(jq -r '.arms | has("check")' <<<"$D")" "false"
assert_eq "check_available=false"  "$(jq -r .check_available <<<"$D")" "false"
assert_eq "check_entrypoint=true"  "$(jq -r .check_entrypoint <<<"$D")" "true"
assert_eq "check_trusted=false"    "$(jq -r .check_trusted <<<"$D")" "false"
D="$(CROSS_REVIEW_TOOL_CHECK_TRUST=1 decide glm "$T/withcheck")"
assert_eq "check arm present with .claude/verify.sh + CROSS_REVIEW_TOOL_CHECK_TRUST=1" "$(jq -r '.arms | has("check")' <<<"$D")" "true"
assert_eq "check_available=true" "$(jq -r .check_available <<<"$D")" "true"
D="$(CROSS_REVIEW_TOOL_CHECK_TRUST=1 decide glm)"
